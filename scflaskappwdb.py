...
def result():
    ...
    sn = result['Sample']
    genes = result['Genes']
    showHighExp = result['highexp']
    showHighVar = result['highvar']
    ...
    print("genelist" + str(set(genelist)))
    print("rmlist" + str(set(rmlist)))
    
    rev_genelist = list(set(genelist) - set(rmlist))
    print("revlist" + str(rev_genelist))
    print(fulldf.iloc[0:5, 0:5])
    cache.set('my-fulldf', fulldf, timeout=10 * 60)
    cache.set('my-genelist', rev_genelist, timeout = 10*60)
    
    if showHighExp == 'highexpcheck':
        dfmeans = fulldf.mean(axis = 0)
        dfsort = dfmeans.sort_values(ascending = False)
        n = result[['expnumber']]
        topgenes = list(dfsort.index[0:n+1])
...
def chooseplots1():
    ...
    print("revlist from cache" + str(genelist))
...
def chooseplots():
    ...
    # make the plot
        if plottype == "violin": 
                ...
                if groupby == "cluster":
                    df.loc[:,"cluster"] = pd.Categorical(df.loc[:, "cluster"], categories = [str(y) for y in range(0,max([int(x) for x in df.loc[:, "cluster"].tolist()]) +1 )], ordered = True)
